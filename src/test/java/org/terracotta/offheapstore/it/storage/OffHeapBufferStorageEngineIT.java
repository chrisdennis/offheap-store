 ...
 */
package org.terracotta.offheapstore.it.storage;
...
import org.terracotta.offheapstore.it.util.OffHeapAndDiskStorageEngineDependentTest.OffHeapTestMode;
import org.terracotta.offheapstore.it.util.ParallelParameterized;
import org.terracotta.offheapstore.it.util.StorageEngineDependentTest;
...
public class OffHeapBufferStorageEngineIT extends StorageEngineDependentTest {
  ...
  public void testPortabilityCacheInvalidation() {
    OffHeapHashMap<Integer, int[]> map = new WriteLockedOffHeapClockCache<Integer, int[]>(new UnlimitedPageSource(new OffHeapBufferSource()), create(new UpfrontAllocatingPageSource(new OffHeapBufferSource(), 1024, 1024), new SerializablePortability(), new SerializablePortability()));
